...
**API:** [`T₀`](@ref), [`T₁`](@ref)–[`T₄`](@ref) — individual analytical terms, unified across both sides of the wavefront (branch automatically on `x` vs. `t`; no separate left/right calls). The complete piecewise analytical solution is assembled from these terms, or obtained directly via [`solve`](@ref)`(`[`ForcedGravityProblem`](@ref)`(...))`. [`gravity_numerical_cpv`](@ref) — direct CPV evaluation. [`gravity_combined_integrand`](@ref) — the $G(k;x,t)$ integrand. [`compute_gravity_profile`](@ref) — batch threaded profile. [`fresnel_C`](@ref) / [`fresnel_S`](@ref) — Fresnel integrals (wraps `FresnelIntegrals.jl`, O(1) cost).

!!! note
    The code sample above uses the legacy `gravity_T0`/`gravity_T1_left`/…/`gravity_analytical_left` wrappers, which remain in the package for backward compatibility but are undocumented. New code should prefer the unified [`T₀`](@ref)–[`T₄`](@ref) functions listed here.
